...
>(({ className, ...props }, ref) => {
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        "open:animate-in closed:animate-out closed:fade-out-0 open:fade-in-0 bg-black/10 duration-100 supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-50",
        className
      )}
      {...props}
    />
  ...
  return (
    <AlertDialogPortal container={container}>
      ...
      <AlertDialogPrimitive.Content
        ...
        data-size={size}
        className={cn(
          "open:animate-in closed:animate-out closed:fade-out-0 open:fade-in-0 closed:zoom-out-95 open:zoom-in-95 bg-background ring-foreground/10 gap-4 rounded-xl p-4 ring-1 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 outline-none",
          className
        )}
        {...props}
      />
