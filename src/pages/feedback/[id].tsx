...
export default function FeedbackDetailPage({
  ...
  const { isAuthor, role, isAdmin } = viewer;
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-foreground">{summary}</h2>
          ...
          <div className="flex gap-2">
            ...
            {isAuthor && <Button type="button">수정 요청</Button>}
            {isAdmin && (status === "pending" || status === "revised_pending") && (
      ...
      <section className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm font-semibold text-amber-500">{ratingStars(rating)}</span>
          <span className="text-xs text-muted-foreground">수정 {revision_count}회</span>
        </div>
        ...
        <div className="mt-5 flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-border/60 bg-muted">
              <Image
                src={avatar_url || PLACEHOLDER_SRC}
                alt={`${display_name} avatar`}
                width={48}
                height={48}
                unoptimized={!avatar_url}
                className="h-full w-full object-cover"
              />
            </div>
            <span className="text-base font-semibold text-foreground">{display_name}</span>
            {is_company_public && company_name && (
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-border/60 bg-white/70 p-4 text-sm text-muted-foreground shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
            ...
            <p className="mt-2 text-base text-foreground">{formatDateTime(created_at)}</p>
          </div>
          <div className="rounded-2xl border border-border/60 bg-white/70 p-4 text-sm text-muted-foreground shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              승인일
            ...
            <p className="mt-2 text-base text-foreground">
              {reviewed_at ? formatDateTime(reviewed_at) : "승인 전"}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{strengths}</p>
        </div>
        <div className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
        ...
        <div className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{suggestions}</p>
        </div>
        <div className="rounded-2xl border border-border/60 bg-background/80 p-6 shadow-sm dark:border-white/10 dark:bg-neutral-900/70">
          <h3 className="text-lg font-semibold text-foreground">승인 정보</h3>
          <p className="mt-3 text-sm text-muted-foreground">
            승인 담당자: <span className="text-foreground">{reviewed_by}</span>
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            마지막 수정: <span className="text-foreground">{formatDateTime(updated_at)}</span>
          </p>
        </div>
      </section>
    </div>
